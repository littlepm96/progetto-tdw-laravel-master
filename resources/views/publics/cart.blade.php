            ...
            <form method="POST" action="{{lang_url('checkout')}}" id="set-order">
                ...
                <div class="products-for-checkout">
                    @foreach($cartProducts as $cartProduct)
                        <?php
                        $sum_total =0;
                        $sum_total += (float)$cartProduct->price * (float)$cartProduct->num_added;
                        $sum = (float)$cartProduct->num_added * (float)$cartProduct->price;
                        ?>
                        <div class="row item">
                            ...
                            <div class="col-sm-2 matchHeight">
                                <a href="javascript:void(0);" onclick="removeProduct({{$cartProduct->id}})"
                                   class="removeProduct remove alignMiddle fa fa-remove"></a>
                            </div>
                            <div class="col-sm-1 matchHeight">
                                <span class="price alignMiddle">{{$cartProduct->num_added}} x {{$cartProduct->price}} = {{ number_format((float)$sum_total) }}</span>
                            </div>
                        </div>
                    @endforeach
                </div>
                <div class="row">
                    @if(session()->has('discount_value'))
                        ...
                        <div class="col-sm-6 total final-total">
                            ...
                            <p>{{__('public_pages.sum_for_pay_discount')}}
                                <span>{{$sum_total - $percentage_value }}</span></p>
                        </div>
                ...
                <div class="row">
                    <div class="col-sm-12 btn-wrap">
                        <a href="checkout"
                           class="btn btn-default"><span>{{__('public_pages.continue_to_checkout')}}</span></a>
                    </div>
                </div>
            </form>
            <div class="row">
                <div class="col-sm-6 promo">
                    <form action="coupon.apply" method="POST">
                        {{ csrf_field() }}
                        <input type="text" placeholder="Coupon" name="coupon_string">
